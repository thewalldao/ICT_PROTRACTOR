...
import { browser, by, ElementFinder, ExpectedConditions as until, Locator, ProtractorBrowser } from "protractor";
...
export default class ElementWrapper {
    ...
    public async waitForVisibilityOfCustom(timeoutInSecond: number = this._elementTimeout, expectTimeInSecond: number, elementName: string = ""): Promise<this> {
        ...
        try {
            ...
            while (stopWatch.getTimeLeftInSecond(timeoutInSecond) > 0 && !isElementDisplayed) {
                if ((stopWatch.getElapsedTimeInSecond() === expectTimeInSecond) && flagPassExpect === 1) {
    ...
    public async getText(timeoutInSecond: number = this._elementTimeout): Promise<string> {
        ...
        try {
            ...
            if (err instanceof error.StaleElementReferenceError) {
                return await this.getText(sw.getTimeLeftInSecond(timeoutInSecond));
            } else {
    ...
    */
    ...
    public async scrollToElementByClass(timeoutInSecond: number = this._elementTimeout): Promise<this> {
        try {
            ...
            throw new errorwrapper.CustomError(this.scrollToElementByClass, err.message);
        }
    }
