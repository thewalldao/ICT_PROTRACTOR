...
export class Utility {
	...
	*/
	...
	// 		configData.browser = xrayJiraTestInfo.Browser;
	...
	// 			if (xrayJiraTestInfo.GBU == "PERM MAXGBU SO32") {
	// 				configData.environment = "USERHUB";
	// 			}
	// 			else {
	// 				configData.environment = "CXONE";
	// 			}
	// 		} else if (xrayJiraTestInfo.Cluster.startsWith("DO") && !xrayJiraTestInfo.Cluster.startsWith("DO91")) {
	// 			configData.environment = "USERHUB";
	// 		} else {
	...
	public static async executeShellCommand(command: string) {
		const { exec } = require("child_process");

		exec(command, (error, stdout, stderr) => {
			if (error) {
				Logger.write(FunctionType.NONE, `${error.message}`);

				return;
			}
			if (stderr) {
				Logger.write(FunctionType.NONE, `stderr: ${stderr}`);
				return;
			}
			Logger.write(FunctionType.NONE, `${stdout}`);
		});
	}
}
...
// 			let getAccessTokenResponse: APIResponse = await Gmail.getAccessToken();
// 			let accessToken: string = JSON.parse(getAccessTokenResponse.body).access_token;
// 			console.log("Get activate link for register account")
// 			let i: number = 0;
...
// 				let getEmailListResponse: APIResponse = await Gmail.getEmailList(accessToken);				
// 				for (let j = 0; j <= 10; j++) {
// 					let message = JSON.parse(getEmailListResponse.body).messages[j];
...
// 							let parts: string = await JSON.parse(getMessageResponse.body).payload.parts;
// 							let email;
// 							for (let part of parts)
// 								if (part.hasOwnProperty("parts"))
// 								{
// 									let sub: object[] = part["parts"];
// 									email = sub.filter((item) => item["mimeType"] == "text/html")[0]
// 									break;
...
// 							let emailBody = await Base64.decode(email.body.data);
// 							let gg = new Gmail();
// 							return await gg.getBodyLink(emailBody);;
// 						}
// 					}
